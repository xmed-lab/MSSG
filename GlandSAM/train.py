...
import matplotlib.pyplot as plt
import copy
from utils.dataset import GlandSAMDataset
...
import cfg
import json
from loss.msg import SemanticContrastiveLoss
# Use the arguments
args = cfg.parse_args()
...
def train_model(trainloader, valloader, dir_checkpoint,epochs):
    ...
    for epoch in pbar:
        ...
        for i,data in enumerate(tqdm(trainloader)):
            ...
            pred, embedding, confi = sam.mask_decoder(
            ...
            msg_loss = msg(embedding, proposal)
            loss =  loss_dice + loss_ce + msg_loss
            ...
            writer.add_scalar('info/msg_loss', msg_loss, iter_num)
...
if __name__ == "__main__":
    ...
    train_dataset = GlandSAMDataset(args,args.img_folder, args.mask_folder, args.proposal_folder, train_img_list,phase='train', targets=[args.targets], normalize_type='sam',if_prompt=False)
    eval_dataset = GlandSAMDataset(args,args.img_folder, args.mask_folder, args.proposal_folder, val_img_list,phase='val', targets=[args.targets], normalize_type='sam',if_prompt=False)
